    ...
    <div class="contact">
      <div class="container centered">
        <div class="contact-list">
          <span class="contact-context">{{seed.contact.info}}</span>
          <a class="link" v-for="item in seed.contact.items" :href="item.to">
            <span v-if="item.icon">
              <font-awesome-icon class="contact-icon" :icon="['fas', item.icon]"/>
            </span>
            <span class="contact-item">{{item.context}}</span>
          </a>
        </div>
        <a class="link" v-for="item in seed.contact.social" :href="item.to"><div ><span><font-awesome-icon class="contact-social" :icon="['fab', item.icon]"/></span></div></a>
      </div>
    </div>
    <div class="nav-main">
      <div class="container">
        <nav class="nav">
          <ul class="nav-bar">
            <li class="nav-item" v-for="item in seed.nav">
            ...
            <li class="nav-item-right" v-for="item in seed.buttons"><a class="link" :href="item.to"><div class="link-item"><span class="nav-context"><font-awesome-icon class="icon" :icon="['fas', item.title]"/></span></div></a></li>
          </ul>
        </nav>
      </div>
    </div>
...
export default {
  data() {
    return {
      seed: {
        contact: {
          info: "Book a Appointment for Free consultation",
          items: [{
            title: "phone",
            icon: "phone",
            context: "99873993",
            ...
            title: "mail",
            icon: "envelope",
            context: "mergenfromrabbit@gmail.com",
            to: "mailto:mergenfromrabbit@gmail.com"
          }],
          social: [
        ...
        nav: [
          ...
          {title: "About", to: "/", active: false},
          {title: "Services", to: "/", active: false},
          {title: "Projects", to: "/", active: false},
          {title: "Blog", to: "/", active: false},
          {title: "Events", to: "/", active: false},
          {title: "Contact Us", to: "/", active: false},
        ],
        buttons: [
          {title: "shopping-bag", to: "/", active: true},
          {title: "search", to: "/", active: false}
        ]
      }
    }
  },
